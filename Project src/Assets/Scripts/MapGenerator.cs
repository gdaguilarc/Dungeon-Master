...
public class MapGenerator : MonoBehaviour {
	...
	void Start () {
		...
		source.PlayOneShot (soundtrack, vol);
		...
		GetComponent<MapAssigner>().Assign(rooms); //passes room info to another script which handles generatating the level geometry
	}

    void CreateRooms(){

		//setup
		rooms = new Roomy[gridSizeX * 2,gridSizeY * 2];
		rooms[gridSizeX,gridSizeY] = new Roomy(Vector2.zero);
		...
		for (int i =0; i < numberOfRooms -1; i++){
			...
			//finalize position
			rooms[(int) checkPos.x + gridSizeX, (int) checkPos.y + gridSizeY] = new Roomy(checkPos);
	...
	void DrawMap(){
		foreach (Roomy room in rooms){
            ...
            Rooms mapper = Object.Instantiate(baseObject, drawPos, Quaternion.identity).GetComponent<Rooms>();
